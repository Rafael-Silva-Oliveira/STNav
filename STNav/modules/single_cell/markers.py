...
def CellTypist_mapper(sc_adata, config, save_path, gene_col, celltype_col):
    ...
    cell_markers_df.to_csv(
        f"{save_path}_CellTypist_markers_{date}.csv",
        index=False,
    )
    ...
    cell_type_markers_df.to_csv(
        f"{save_path}_CellTypist_markers.csv",
        index=False,
    )
    return cell_type_markers_dict, cell_type_markers_df


def SCVI_mapper(sc_adata, config, save_path, gene_col, celltype_col):
    ...
    de_df.to_csv(
        f"{save_path}_scVI_DEG_{date}.csv",
        index=False,
    )
    ...
    with plt.rc_context():  # Use this to set figure params like size and dpi
        ...
        plt.savefig(
            f"{save_path}_heatmap_top_markers_SCVI_{date}.png",
            bbox_inches="tight",
        )
        plt.close()
    ...
    # Convert dictionary to DataFrame
    df = pd.DataFrame(
        dict([(key, pd.Series(value)) for key, value in cell_type_markers_dict.items()])
    )
    ...
    cell_type_markers_df.to_csv(
        f"{save_path}_scVI_markers.csv",
        index=False,
    )
    return cell_type_markers_dict, cell_type_markers_df


def scMAGS_mapper(sc_adata, config, save_path, gene_col, celltype_col):

    # Add assertion that checks if cell_annotation_col is in sc_adata.obs.columns
    cell_annotation_col = config["labels"]
    assert (
    ...
    cell_type_markers_df.to_csv(
        f"{save_path}_scMAGS_markers.csv",
        index=False,
    )
    return cell_type_markers_dict, cell_type_markers_df


def perform_marker_extraction(config, sc_adata, save_path):
    ...
    if config["scMAGS"]["usage"]:
        cell_type_markers_scMAGS_dict, cell_type_markers_scMAGS_df = scMAGS_mapper(
            sc_adata=sc_adata,
            config=config["scMAGS"],
            save_path=save_path,
            gene_col=gene_markers_col_name,
            celltype_col=celltype_col_name,
        )
...
config_dict = {
    ...
    "SCVI": {
        ...
        "max_epochs": 25,
        "early_stopping": True,
        "pre_trained_model_path": "/mnt/work/RO_src/models/scmodel",
        "markers_save_path": "/mnt/work/RO_src/data/processed",
    },
    "CellTypist": {
        "usage": True,
        "train": True,
...
st_adata = sc.read_h5ad(
    r"/mnt/work/RO_src/STAnalysis/notebooks/experimental/B2C/adata_b2c.h5ad"
)

st_adata_subset, sc_adata_subset, intersection = get_intersect(
    sc_adata=adata_to_annotate, st_adata=st_adata
)


markers = perform_marker_extraction(
    config=config_dict,
    sc_adata=sc_adata_subset,
    save_path="/mnt/work/RO_src/STAnalysis/notebooks/downstream/scRNA/markers",
)
